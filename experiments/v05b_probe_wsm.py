...
for k,e in expct.items():
 d=dist(*k);m=(d<=SR)&(azi>=0)&(azi<180);data.append((k,azi[m],d[m],qual[m],e))
maps={
 'none':{},
 'Stress2Grid':{'A':1.6666667,'B':1.25,'C':1.0,'D':0.625,'E':0.0},
 'linear':{'A':1.0,'B':.8,'C':.6,'D':.4,'E':.2},
 'soft':{'A':1.0,'B':.9,'C':.8,'D':.7,'E':.5,'XMI':.3,'XRU':.3,'XNE':.3},
 'abcd_equal_drop_special':{'A':1,'B':1,'C':1,'D':1,'E':0},
}
def quality_w(qs,mp):
 if not mp:return np.ones(len(qs))
 return np.array([mp.get(q,0.0) for q in qs],float)
def score(sig,mp):
 ae=[];re=[]
 for k,v,d,q,e in data:
  w=np.exp(-(d/sig)**2)*quality_w(q,mp)
  if w.sum()==0:w=np.exp(-(d/sig)**2)
...
for name,mp in maps.items():
 vals=[(score(float(s),mp),float(s)) for s in np.arange(500,901,5)];vals.sort(key=lambda x:x[0]);best,sig=vals[0];print('\nBEST',name,'sigma',sig,'score',best)
 for k,v,d,q,e in data:
  w=np.exp(-(d/sig)**2)*quality_w(q,mp)
  if w.sum()==0:w=np.exp(-(d/sig)**2)
